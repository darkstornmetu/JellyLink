...
public class Grid<T> where T : class, IGridItem 
{
    ...
    [SerializeField] Vector3[] _gridPositions;
    [SerializeField] bool[] _unavailabilityArray;
    ...
    public Grid(int width, int height)
    {
        ...
        _gridPositions = new Vector3[width * height];
        _unavailabilityArray = new bool[width * height];
    }
    ...
    {
        return !IsValidCoordinates(coordinates) ? default : 
            _gridPositions[GetIndex(coordinates)];
    } 

    public void SetGridPosition(Vector2Int coordinates, Vector3 pos)
    {
        if (!IsValidCoordinates(coordinates)) return;

        _gridPositions[GetIndex(coordinates)] = pos;
    }

    public void SetUnavailability(Vector2Int coordinates, bool isUnavailable)
    {
        if (!IsValidCoordinates(coordinates)) return;

        _unavailabilityArray[GetIndex(coordinates)] = isUnavailable;
    }
    
    public bool GetUnavailability(Vector2Int coordinates)
    {
        return !IsValidCoordinates(coordinates) ? default : 
            _unavailabilityArray[GetIndex(coordinates)];
    }
